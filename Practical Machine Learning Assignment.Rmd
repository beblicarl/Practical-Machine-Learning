...
library(lattice); library(ggplot2); library(caret); library(randomForest); library(rpart); library(rpart.plot);library(rattle);library(RColorBrewer)
```
...
```{r}
u <- union(predictionsA1, myTesting$classe)
t <- table(factor(predictionsA1, u), factor(myTesting$classe, u))
confusionMatrix(t)
```
...
```{r}
m <- union(predictionsB1, myTesting$classe)
n <-table(factor(predictionsB1, m), factor(myTesting$classe, m))
confusionMatrix(n)
```
